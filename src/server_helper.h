...
const char *pflagsfname = "/home/ubuntu/proj/dsa110-shell/dsa110-nsfrb/process_server/process_flags.txt";
...
//read and reset flag status
char get_pflags() {
	FILE *pflagsf;
	pflagsf = fopen(pflagsfname,"r");
	char pflags;
	fread(&pflags,1,1,pflagsf);
	fclose(pflagsf);
	return pflags;
}
...

{
	...
	if (argc == 0)
        ...
        else
        {
			...
			{
				tostdout = 1;
				fprintf(logfile,"Will pipe to stdout\n");
			}
