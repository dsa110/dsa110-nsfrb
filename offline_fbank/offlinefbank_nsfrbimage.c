...
#include "dsaX_def.h"
#include "offlinefbank_nsfrbimage.h"
...
cmdargs *parseargs(cmdargs *args, int argc, char *argv[])
{
	...
	args->logfile = "";
        ...
        while (i < argc)
        {
                ...
                {
			if (strcmp(argv[i] + 2,"fnum") == 0) {args->fnum = argv[i+1];}
			else if (strcmp(argv[i] + 2,"datapath") == 0) {args->datapath = argv[i+1];}
			else if (strcmp(argv[i] + 2,"gulp") == 0) {args->gulp = atoi(argv[i+1]);}
                        else if (strcmp(argv[i] + 2,"ngulps") == 0) {
				if (atoi(argv[i+1])<=90 && atoi(argv[i+1])>=args->gulp )
				{
					args->ngulps = atoi(argv[i+1]);
				}
			}
			else if (strcmp(argv[i] + 2,"sb") == 0) {args->sb = atoi(argv[i+1]);}
		...
		double *BLEN, double bmin, double bmax)
{
	...
	for (int chann=0; chann<num_chans_per_node;chann++)
	{
		...
		for (int k=0; k<nbase; k++)
			...
			/*
			for (int k_t=0; k_t<num_time_samples; k_t++)
			{
				vis_grid[grid_index + (k_t*num_chans_per_node*gridsize*gridsize)][0] += bweight_complex[0]*(data[data_index + (k_t*nbase*num_chans_per_node*2)][0] + 
															data[data_index+(k_t*nbase*num_chans_per_node*2)+1][0]);
				vis_grid[grid_index + (k_t*num_chans_per_node*gridsize*gridsize)][1] += bweight_complex[0]*(data[data_index + (k_t*nbase*num_chans_per_node*2)][1] + 
															data[data_index+(k_t*nbase*num_chans_per_node*2)+1][1]);
				vis_grid[grid_conj_index + (k_t*num_chans_per_node*gridsize*gridsize)][0] += bweight_complex[0]*(data[data_index + (k_t*nbase*num_chans_per_node*2)][0] +
                                                                                                                        data[data_index+(k_t*nbase*num_chans_per_node*2)+1][0]);
                                vis_grid[grid_conj_index + (k_t*num_chans_per_node*gridsize*gridsize)][1] -= bweight_complex[0]*(data[data_index + (k_t*nbase*num_chans_per_node*2)][1] +
                                                                                                                        data[data_index+(k_t*nbase*num_chans_per_node*2)+1][1]);
			}
			*/
			//index
...
int main(int argc, char *argv[])
{
	...
	const char *nsfrbdata = getenv("NSFRBDATA");
	

	
	//data file
	char sb_s[6];
	sprintf(sb_s,"sb%02d_",args->sb);
	char datafname[300];
	strcpy(datafname,nsfrbdata);
	strcat(datafname,"/dsa110-nsfrb-fast-visibilities/");
	strcat(datafname,args->datapath);
	strcat(datafname,"/nsfrb_");
	strcat(datafname,sb_s);
	strcat(datafname,args->fnum);
	strcat(datafname,".out");
	printf("%s\n",datafname);
	...
	float dec=0;
	double mjd=0;
        FILE *fobj_data = fopen(datafname,"rb");
	size_t nread;
	nread = fread(&mjd,sizeof(double),1,fobj_data)*sizeof(double);
        nread += fread(&sb,sizeof(float),1,fobj_data)*sizeof(float);
        nread += fread(&dec,sizeof(float),1,fobj_data)*sizeof(float);
	printf("MJD=%f;SB=%f;DEC=%f\n",mjd,sb,dec);

	char gridfname_prefix[300];
	char gridfname[300];
	char imgfname[300];
	char imgfname_prefix[300];
	char testfname[300];
        char testfname_prefix[300];
        strcpy(imgfname_prefix,nsfrbdata);
        strcat(imgfname_prefix,"/dsa110-nsfrb-fast-visibilities/");
	strcat(imgfname_prefix,args->datapath);
	//strcpy(imgfname_prefix,cwd);
	//strcat(imgfname_prefix,"//offline_fbank");
        strcat(imgfname_prefix,"/nsfrb_");
        strcat(imgfname_prefix,sb_s);
        strcat(imgfname_prefix,args->fnum);
	strcpy(gridfname_prefix,imgfname_prefix);
	strcpy(testfname_prefix,imgfname_prefix);
	strcat(imgfname_prefix,"_ofbimage_");
	strcat(gridfname_prefix,"_ofbgrid_");
	strcat(testfname_prefix,"_ofbtest_");


	if (uselogfile)
        {
                fobj = fopen(args->logfile,"a");
                fprintf(fobj,"retrieved filename=%s\n",imgfname_prefix);
                fclose(fobj);
        }
        else{
        ...
        strcat(baseUVWcmd, "/offline_fbank/_getbaselines.py --pt_dec ");
	

	char searchstage[300];
	strcpy(searchstage,cwd);
	strcat(searchstage,"/offline_fbank/searchstage.txt");


        char ufname[300];
        strcpy(ufname,cwd);
        strcat(ufname,"/offline_fbank/U.bin");

        char vfname[300];
        strcpy(vfname,cwd);
        strcat(vfname,"/offline_fbank/V.bin");

        char wfname[300];
        strcpy(wfname,cwd);
        strcat(wfname,"/offline_fbank/W.bin");

        char bfname[300];
        strcpy(bfname,cwd);
        strcat(bfname,"/offline_fbank/BLEN.bin");

        char a1fname[300];
        strcpy(a1fname,cwd);
        strcat(a1fname,"/offline_fbank/ANT1.bin");

        char a2fname[300];
        strcpy(a2fname,cwd);
        strcat(a2fname,"/offline_fbank/ANT2.bin");
	...
	*/
	...
	memset(data, 0, sizeof(fftwf_complex)*total_samples);
	memset(data_conj,   0, sizeof(fftwf_complex)*total_samples);
	...
	fseek(fobj_data, sizeof(fftwf_complex)*total_samples*(args->gulp),SEEK_CUR);
	char gulp_s[11];
	for (int g=(args->gulp); g<(args->gulp)+(args->ngulps); g++)
	{
		//t1 = myCPUTimer();
		if (uselogfile)
                {
		...
		// read block
		nread = fread(data,sizeof(fftwf_complex),total_samples,fobj_data)*sizeof(fftwf_complex);
		if (args->save)
                {

                        strcpy(testfname,testfname_prefix);
                        sprintf(gulp_s,"gulp%02d.out",g);
                        strcat(testfname,gulp_s);


                        fobj_w = fopen(testfname,"wb");
                        fwrite(data, sizeof(fftwf_complex), (args->num_time_samples)*(args->max_base)*(args->num_chans_per_node)*2, fobj_w);
                        fclose(fobj_w);
		...
		if (args->save)
		{
			strcpy(imgfname,imgfname_prefix);
        		sprintf(gulp_s,"gulp%02d.out",g);
			strcat(imgfname,gulp_s);

			strcpy(gridfname,gridfname_prefix);
                        sprintf(gulp_s,"gulp%02d.out",g);
                        strcat(gridfname,gulp_s);
			...
			fobj_w = fopen(gridfname,"wb");
                        fwrite(vis_grid, sizeof(fftwf_complex), (args->num_time_samples)*(args->gridsize)*(args->gridsize)*(args->num_chans_per_node), fobj_w);
                        fclose(fobj_w);

			if (args->search)
			{
				fobj_w = fopen(searchstage,"a");
				fwrite(strcat(imgfname,"\n"), sizeof(char), strlen(imgfname)+1, fobj_w);
				fclose(fobj_w);
			}
		}
		if (uselogfile)
