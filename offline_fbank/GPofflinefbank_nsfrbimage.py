...
def main(args):
    ...
    for gulp in gulps:
        ...
        #read from file
        dat,sb,mjd_init,dec = pipeline.read_raw_vis(datadir+"nsfrb_sb{a:02d}_{b}.out".format(a=args.sb,b=args.fnum),get_header=False,nsamps=args.num_time_samples,nchan=args.nchans_per_node,gulp=gulp,headersize=16)
        ...
        np.save(datadir+"nsfrb_sb{a:02d}_{b}_ofbimage_gulp{c:02d}_{d}.npy".format(a=args.sb,b=args.fnum,c=gulp,d=Time(mjd,format='mjd').isot),dirty_img)

        fsrch=open(GPofflinestagefile,"a")
        fsrch.write(datadir+"nsfrb_sb{a:02d}_{b}_ofbimage_gulp{c:02d}_{d}.npy\n".format(a=args.sb,b=args.fnum,c=gulp,d=Time(mjd,format='mjd').isot))
        fsrch.close()
    executor.shutdown()
    return
...
if __name__=="__main__":
    ...
    if len(args.fnum)==0:
        allfs = np.sort(glob.glob(vis_dir + args.GPdir + "/nsfrb_sb{a:02d}_*.out".format(a=args.sb)))
        print("Searching all data from "+args.GPdir)
        for fs in allfs:
            args.fnum=os.path.basename(fs)[os.path.basename(fs).index("sb{a:02d}_".format(a=args.sb))+5:-4]
            print("<<<"+args.fnum+">>>")
            print("-"*100)
            print("")
