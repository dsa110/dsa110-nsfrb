...
def generate_PSF_images(dataset_dir,dec,zoom_pix,tonumpy,nsamps=1,dtype=np.float32,HA=0,injectnoise=0,noise_only=False,srcDECoffset=0,srcHAoffset=0,flagged_antennas=flagged_antennas,bmin=bmin,robust=robust,RFI=False,rfi_type="far"):
    num_observations = 1
    ...
    PSF = np.array(generate_src_images(dataset_dir, num_observations, noise_std_low, noise_std_high, exclude_antenna_percentage, HA_point, HA_source, Dec_point, Dec_source, spectral_index_low, spectral_index_high, zoom_pix, tonumpy,inflate=zoom_pix>IMAGE_SIZE//2,noise_only=noise_only,N_NOISE=nsamps,flagged_antennas=flagged_antennas,bmin=bmin,robust=robust,RFI=RFI,rfi_type=rfi_type),dtype=dtype)
