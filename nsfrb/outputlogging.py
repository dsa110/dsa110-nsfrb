...
"""
from nsfrb.config import run_file,error_file
...
def send_candidate_slack(filename,filedir,error_file=error_file):
    #upload file to bot
    try:
...
import requests
import os
def send_candidate_pushover(filename,filedir,error_file=error_file):
    try:
...
def send_candidate_custom(filename,port=9087,error_file=error_file):
    try:
