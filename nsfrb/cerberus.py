...
@jax.jit
def realtime_robust_image_jit_lowmem(weighted_V, visibility_grid, t_indices, i_indices,j_indices,i_conj_indices,j_conj_indices,tidx):
    """
    ...
    print("step 1")
    print(t_indices.shape,i_indices.shape,j_indices.shape,weighted_V.shape,visibility_grid.shape)
    visibility_grid = visibility_grid.at[t_indices,
                                         i_indices,
                                         j_indices].add(weighted_V.flatten())
    visibility_grid = visibility_grid.at[t_indices,
                                         i_conj_indices,
                                         j_conj_indices].add(jnp.conj(weighted_V.flatten()))
...
def matched_filter_dedisp_snr_fft_jit_completeness(image_tesseract_input,PSFimg,corr_shifts_all,tdelays_frac,boxcar,noise,past_noise_N,noiseth):
    ...
    image_tesseract_binned_new = (image_tesseract_binned.at[:,:,:,:,0].set((jnp.abs( ((image_tesseract_binned.max(4))/jnp.expand_dims(noise[:,np.newaxis].repeat(nDM,1),(1,2)))))))[:,:,:,:,0].transpose(1,2,0,3)
...
def img_diff_jit_no_append(image_tesseract_input,PSFimg,boxcar,noise,past_noise_N,noiseth):
    ...
    image_tesseract_binned_new = (image_tesseract_binned.at[:,:,:,:,0].set((jnp.abs( ((image_tesseract_binned.max(4) - jnp.nanmedian(image_tesseract_binned*mask,axis=4))/jnp.expand_dims(noise[:,np.newaxis].repeat(1,1),(1,2)))))))[:,:,:,:,0].transpose(1,2,0,3)
...
def img_diff_jit_no_append_completeness(image_tesseract_input,PSFimg,boxcar,noise,past_noise_N,noiseth):
    ...
    image_tesseract_binned_new = (image_tesseract_binned.at[:,:,:,:,0].set((jnp.abs( ((image_tesseract_binned.max(4))/jnp.expand_dims(noise[:,np.newaxis].repeat(1,1),(1,2)))))))[:,:,:,:,0].transpose(1,2,0,3)
    del mask
    del boxcar
