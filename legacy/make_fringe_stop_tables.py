...
def main(args):
    ...
    #define time axis
    phase_time_axis = np.array([0])#we only need to compute this once, shouldn't depend on RA np.arange(0,24*3600,args.phase_interval)#np.arange(0,(1/60)*3600,args.phase_interval)
    fringe_time_axis = np.arange(0,args.chunk_interval,args.fringe_interval) #should be 25 samples (130 ms) from 0 to 3.25 s
    ...
    if not args.usetemplate:
        #make UVW grid
        u,v,w = cal.antpos_to_uv(core_only=False)
        u = np.array(u)
        v = np.array(v)
        w = np.array(w)
        ...
        for dec_center in dec_axis:
            ...
            for phase in phase_time_axis:
                i = 0
                #set ra of phase center
                ra_center = phase*15/3600
                for fringe in fringe_time_axis:
                    #set ra of current pointing
                    ra_point = ra_center + (fringe*np.cos(dec_center*np.pi/180)*15/3600)
                    dec_point = dec_center

                    if args.verbose: print(u,v,w)
...
if __name__=="__main__":
    ...
    parser.add_argument('--usetemplate',action='store_true',help='If set, uses uvw coordinates from hdf5 files. Otherwise, uvw are generated manually from antenna positions.')
    ...
    parser.add_argument('--dec_interval',type=float,help='Declination interval for which phase correction is assumed constant,default=10 degrees',default=3.2)
