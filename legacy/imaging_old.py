...
def deredden(img,chanbw,R2002=False,R2002nbins=16,returnFFT=False,keepDC=True,scalefactor=4,sincfit=False,sincfobs=None,sincpeak=None):
    """
    ...
    if R2002 and not sincfit:
        ...
        return x_filt
    elif sincfit and sincfobs is not None and sincpeak is not None:
        assert(len(sincpeak)<len(img.shape))
        #try fitting a sinc function and normalizing out
        #img_spec =np.reshape(img_spec,(np.nanprod(img.shape[:-1]),img.shape[-1]))
        #get spectrum from given peak; other axes will be averaged over
        img_spec = img[sincpeak]
        if len(img_spec.shape)>1:
            img_spec = np.nanmean(img_spec,axis=tuple(np.arange(len(img_spec.shape)-1)))
        assert(len(img_spec)==len(sincfobs))
        
        
        popt,pcov = curve_fit(lambda x,b,A,c: A*np.sinc(b*x - c),sincfobs,img_spec,p0=(np.nanmax(img_spec),1,0))
        print("Sinc fit:",popt)
        return img/np.sinc(popt[0]*sincfobs - popt[2])
    else:
        ...
        if wstack and w is not None:
            ...
            for i in range(v_avg.shape[0]):
                ...
                np.add.at(visibility_grid_i, (np.concatenate([i_indices[nancondition],i_conj_indices[nancondition]]),
                                    ...
                                    np.concatenate([k_indices[nancondition],k_conj_indices[nancondition]])),
