...
from nsfrb.simulating import compute_uvw, add_complex_gaussian_noise, get_core_coordinates, apply_spectral_index
from nsfrb.imaging import uniform_image
from nsfrb.config import NUM_CHANNELS, CH0, CH_WIDTH, AVERAGING_FACTOR, IMAGE_SIZE, c


def generate_src_images(dataset_dir, num_observations, noise_std_low, noise_std_high, exclude_antenna_percentage, HA_low, HA_high, Dec_low, Dec_high, spectral_index_low, spectral_index_high, zoom_pix, tonumpy,inflate=False,noise_only=False,N_NOISE=1):
    """
    ...
    - HA_low (float): Lower bound for HA (Hour Angle).
    - HA_high (float): Upper bound for HA (Hour Angle).
    - Dec_low (float): Lower bound for Dec (Declination).
    - Dec_high (float): Upper bound for Dec (Declination).
    ...
    for obs in tqdm(range(num_observations), desc="Progress"):
        ...
        u_core, v_core, w_core = compute_uvw(x_core, y_core, z_core, HA, Dec) #core antennas
...
if __name__ == "__main__":
    ...
    parser.add_argument('--HA_low', type=float, default=0, help='Lower bound for HA')
    parser.add_argument('--HA_high', type=float, default=10/180*np.pi, help='Upper bound for HA')
    parser.add_argument('--Dec_low', type=float, default=-np.pi/2, help='Lower bound for Dec')
    parser.add_argument('--Dec_high', type=float, default=np.pi/2, help='Upper bound for Dec')
    ...
    generate_src_images(args.dataset_dir, args.num_observations, args.noise_std_low, args.noise_std_high, args.exclude_antenna_percentage, args.HA_low, args.HA_high, args.Dec_low, args.Dec_high, args.spectral_index_low, args.spectral_index_high, args.zoom_pix,args.tonumpy,args.inflate,args.NNOISE)
