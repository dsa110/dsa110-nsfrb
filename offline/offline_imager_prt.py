...
#flagged antennas/
TXtask_list = []
def pseudo_realtime_image_task(dat, tidx, U_wavs, V_wavs, i_indices_all, j_indices_all, i_conj_indices_all, j_conj_indices_all, bweights_all, gridsize,  pixel_resolution, nchans_per_node, fobs_j, jj, briggs=False, robust= 0.0, return_complex=False, inject_img=None, inject_flat=False, wstack=False, W_wavs=None,  Nlayers_w=18,pixperFWHM=pixperFWHM,wstack_parallel=False,sb=0):#,port=-1,ipaddress="",time_start_isot="", uv_diag=-1, Dec=-1, TXexecutor=None, stagger=0):
...
def main(args):
    ...
    while True:
        dirty_img = np.zeros((args.gridsize,args.gridsize,args.num_time_samples,nchans))
        for sb_i in range(nchans):
            dat = None
            ...
            if np.abs((Dec*np.pi/180) - pt_dec)>1e-2:
                printlog("updating UVWs for dec " + str(Dec),output_file=logfile)
                pt_dec = Dec*180/np.pi
                bname, blen, UVW = pu.baseline_uvw(antenna_order, pt_dec, refmjd, casa_order=False)
                ...
                if injection_params is None:
                    ...
                    if time_start_isot == injection_params['ISOT']:
                        if args.testh23:
                            ...
                            injection_params["injected"][sb] = True
                    ETCD.put_dict(ETCDKEY_INJECT,injection_params)

                    if time_start_isot == injection_params['ISOT']:
                        #if verbose: printlog("Injection" + injection_params['ID'] + "found",output_file=logfile)
                        fname = "injection_" + str(injection_params['ID']) + "_sb" +str("0" if sb<10 else "")+ str(sb) + ".npy"
                        #copy
                        if args.testh23:
                            os.system("cp " + inject_dir + "realtime_staging/" + "injection_" + str(injection_params['ID']) + "_sb" +str("0" if sb<10 else "")+ str(sb) + ".npy " + local_inject_dir)
                        else:
                            os.system("scp h24.pro.pvt:" + inject_dir + "realtime_staging/" + "injection_" + str(injection_params['ID']) + "_sb" +str("0" if sb<10 else "")+ str(sb) + ".npy " + local_inject_dir)
            ...
            for j in range(args.nchans_per_node):
                ...
                for tidx in range(5):
                    task_list.append(executor.submit(pseudo_realtime_image_task,dat[tidx*5:(tidx+1)*5,:,j,:],
                                                    tidx,
                                                    U_wavs[:,jj],
                                                    V_wavs[:,jj],
                                                    i_indices_all[:,jj],
        ...
        for t in task_list:
            m=t.result()
            dirty_img[:,:,m[1]*5:(m[1]+1)*5,m[2]] += m[0] #t.result()
        ...
        #send to proc server
        if args.save:
            np.save(img_dir + time_start_isot + "_prtimage.npy",dirty_img)
        if args.search:
            for sbi in range(len(corrs)):
                ttx = time.time()
