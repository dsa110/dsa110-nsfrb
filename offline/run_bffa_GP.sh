...
do
	...
	if ([[ "$runfile" == "all" ]] || ([[ "${runfile:0:1}" == "_" ]] && [[ "$label" == "$runfile" ]]) || ([[ "${runfile:0:1}" != "_" ]] && [[ "$labeldate" == "${runfile:0:10}" ]] && [[ "$(date -d "${runfile}" +%s)" -lt "$labelseconds" ]])) && (( $count < $maxrun )); then
		...
		if (( $prerun==0 )); then
			echo ${label: 1:6}
			python ffa_search.py ${label: 1:6} --path $filedir --bmin 20 --mingulp $4 --ngulps $ngulp --widthtrials $(seq 1 50) --periods $(seq 50 600) --ignoreSNR --flagchans 63 64 65 24 23 84 92 101 106 107 108 110 111 114 115 116 118 119 120 121 122 124 125 126 51 --showprofiles 1 --finePrange 1 --nfinePtrials 50 --mode beamform_search --gridcols 6 --gridrows 8 --gridstep 0.125
