...
do
	...
	if ([[ "$runfile" == "all" ]] || ([[ "${runfile:0:1}" == "_" ]] && [[ "$label" == "$runfile" ]]) || ([[ "${runfile:0:1}" != "_" ]] && [[ "$labeldate" == "${runfile:0:10}" ]] && [[ "$(date -d "${runfile}" +%s)" -lt "$labelseconds" ]])) && (( $count < $maxrun )); then
		...
		if (( $prerun==0 )); then
			python /home/ubuntu/msherman_nsfrb/DSA110-NSFRB-PROJECT/dsa110-nsfrb/offline/offline_imager.py $label --verbose --offline --num_gulps $ngulp --gulp_offset $4 --num_time_samples 5 --sb --nchans_per_node 8 --filedir $filedir$injectflag --num_inject $inject --width_inject 2 --inject_noiseless --gridsize 601 --flagBPASS --flagBPASSBURST --sleeptime 0 --offsetRA_inject 0 --offsetDEC_inject 0 --briggs --robust 2 --search
