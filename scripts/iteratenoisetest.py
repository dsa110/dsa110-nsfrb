...
with open("dm0calcurve.pkl","rb") as p:
    cc = pkl.load(p)
allnoise = []
for SNR in np.arange(11,12)[::-1]:
    noise_inj = np.load(config.img_dir + "senstest_noise.npy") #np.zeros(5):
    #snrsamps_inj_full = np.zeros((8,5,16,gridsize,gridsize-RA_cutoff,5,16))
    snrsamps_inj_full = np.zeros((8,5,16,5,16))

    for width_i in [0,4]:#range(5):
        injloc = 0.5 - ((2**width_i)/nsamps/2)
        for dm_i in [0,15]:#range(16):
            ...
            for i in range(int((18+8*SNR)%(len(allfs)-8))+1,int((18+8*SNR)%(len(allfs)-8))+9):#(40,48):#1,len(allfs)//6):
                print(">>>",i,"SIGNAL-->",(np.nanmax(inject_img)/cc(SNR)))
                ...
                print("")
                allnoise.append(noise_inj)
    #np.save(config.img_dir + "senstest_snrsamps_inj.npy",snrsamps_inj_full)
    #np.save(config.img_dir + "senstest_snrsamps_inj_SNR"+str(SNR)+".npy",snrsamps_inj_full)
    #np.save(config.img_dir + "senstest_noise_inj_SNR"+str(SNR)+".npy",noise_inj)
np.save(config.img_dir+"senstest_noise_inj_all.npy",np.array(allnoise))
