...
def process_data_sb(fname, num_gulp, num_time_samples=25, verbose_flag=False, plot_uv_analysis_flag=False, plot_dirty_images_flag=False, num_channels=NUM_CHANNELS,averaging_factor=AVERAGING_FACTOR,image_size=IMAGE_SIZE,num_batches=1):
    ...
    dirty_images_all_concat = np.zeros((image_size,image_size,num_time_samples*num_gulp,num_channels//averaging_factor))
    for gulp in tqdm(range(num_gulp), desc="Processing gulps"):
        ...
        dirty_images_all_concat[:,:,gulp*num_time_samples:(gulp+1)*num_time_samples,:] = dirty_images_all.transpose((2, 3, 0, 1))
    ...
    tb.close()
    return dirty_images_all_concat
