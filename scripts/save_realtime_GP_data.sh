...
gpname=${1}
gptime=${gpname:18}
cpdir="/dataz/dsa110/nsfrb/dsa110-nsfrb-fast-visibilities/GP_observations_${gptime}/"
echo "saving data to ${cpdir}"
while :
      do

	  for i in ${!corrs[@]}; do
	      rsync -avv --remove-source-files ${corrs[$i]}.pro.pvt:./data/nsfrb_*.out ${cpdir} > tmp1.txt
